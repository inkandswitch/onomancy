...
The binding cache is a grow-only **store** of self-authenticating protocol records, paired with a user-private **judgment document** carrying the user's decisions. Every piece of verifier state is a deterministic **derivation** — `derive(store, now, judgment)` — so record synchronization is plain set union, judgment synchronization is ordinary document replication, and arrival order is never an input. This document defines the store, the judgment document, the derivation, surfacing, and pruning.
...
- **Sync is set union.** Any transport that unions records is a valid sync protocol — gossip already is one. Pooling records with a group needs no new trust analysis: records carry their own proof, and judgment never travels with them — it lives in its own access-controlled document.
...
The store holds **records** — append-only, merged by set union, accepted from **anyone**: certificates with their attached regions (including superseded ones), refreshed DNSSEC chains and the TXT records they carry, rotation and successor statements (usually carried inside certificates), and proven-absence records. The user's own decisions live elsewhere: in [the Judgment Document], not the store.
...
`derive(store, now, judgment)` computes, per hostname: the **accepted binding** (document + generation key + grade), the **effective serial**, **tenure**, **lineage** (with forks), the **pending** and **contested** sets, **unbound** status, and **divergence** badges. Its inputs are three — the store, the clock reading, and the state of [the Judgment Document] — plus the user's pinned targets, read by stage 8's divergence badges only. It MUST be deterministic: the same inputs yield the same outputs on any device, in any implementation; the judgment document's state converges across devices by CRDT replication, so verdicts converge with it.
...
1. **Validate and extract.** Discard undecodable items and invalid statements ([DNS Anchoring] statement validity, D17); extract carried statements as items ([The Store]); read the judgment document's entries (authenticity and privacy are the document's own access control — nothing to filter here). Chains that never verify from the KSK are invalid ✗ and discarded. Lineage chain-shape violations (double-replace, double-successor, cycles — including generation-key reuse) are NOT discarded here: they are set-wise **forks**, evaluated at stage 3 ([DNS Anchoring] D18).
...
8. **Absence and divergence.** A proven-absence record is compared to binding records by the **inception of the leaf RRSIG** — the NSEC/NSEC3 records' own signature vs the TXT RRset's own signature, not the chain ∩-window, whose start moves with unrelated parent re-signing: absence whose leaf inception is **strictly later** than every surviving binding record's yields **unbound** (surfaced; the previously accepted binding remains derivable as the succession anchor); equal or earlier inception contributes nothing — on overlap, the binding wins — and stale absence proofs prove only past absence. Finally, claims and pinned targets are compared against the accepted binding; mismatches yield badges per [Petname Anchoring][Divergence and Re-Pin].

# Surfacing
[Surfacing]: #surfacing

State is what `derive` returns; **events are diffs**. When a store update or the passage of time changes the derivation output, implementations MUST surface the difference, following the events-vs-states doctrine ([DNS Anchoring], Staleness in Practice): binding changes, ratchet resets, forks, and unbinding are events (shown, possibly prompting); badge appearances and clearances (pending, contested, provisional, staleness) are visible state changes that MUST NOT prompt — "silently" anywhere in this document means "without a prompt," never "invisibly." The only prompt the pending or contested condition may generate is at use time — proceed on the accepted/pinned binding, or wait — a risk decision about the user's own action, never an invitation to adjudicate evidence (B4).

# The Judgment Document
[the Judgment Document]: #the-judgment-document

The user's decisions live in a **judgment document**: a user-private [Keyhive] document, linked from the root document, with access delegated to exactly the user's devices. It is not a new mechanism — it is the substrate doing what the substrate does, and each property the retired statement machinery hand-built is inherited structurally:

| Property                                           | Provided by                                                                                                                     |
|----------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------|
| Authentication (only your devices write judgment)  | The document's write delegation — what "enrolled device keys" meant, enforced by Keyhive rather than by verifier-side filtering |
| Privacy (judgment never reaches peers or groups)   | E2EE + device-only read access — structural, not a behavioral MUST                                                              |
| Sync (decisions follow the person, not the device) | Ordinary document replication — the private channel judgment always needed                                                      |
...
> [!NOTE]
> A judgment document shared beyond one user's devices — a team accepting bindings together — is expressible with the same mechanism and deliberately out of scope at v0: it is delegated trust with real UX consequences, and it changes nothing in the derivation, which simply reads whatever judgment document it is given.
...
- `derive` is a pure function of `(store, now, judgment)`. Determinism — same inputs, same verdicts — is a conformance and formal-verification target (it extends [design/verification.md](../../design/verification.md) target 7 from pooled-evidence verdicts to the full derivation).
...
|-----|-----------|------------------------|
| B1  | Stale ⚠ record attests a document other than the judgment-backed incumbent, no valid succession proof | Pending: badge, no prompt; never displaces the incumbent |
...
| B8  | Reset entry | Its excluded items (with their extraction closure) contribute to no derivation output at their natural scope; MUST be manual; undone by editing the judgment document; evidence outside the excluded set survives and is surfaced |
| B9  | Invalid rotation/successor statement ([DNS Anchoring] D17) | Discarded at validation; no lineage effect, never fork evidence |
| B10 | Sole or ladder-maximal candidate, no acceptance in the judgment document, only stale evidence | Incumbent, graded provisional |
| B11 | Fork or competing statement implicating provisional support | Output demotes automatically; surfaced |
| B12 | Fresh ✓ proven absence (D2) | Unbound event: surfaced; prior accepted binding remains the succession anchor; stale absence inert |
