...
Fixed-width cryptographic material (keys, hashes, signatures) is encoded raw, without length prefixes, at the widths given below. Fields typed as ed25519 verifying keys — document IDs, signers, generation keys — MUST decode to valid curve points **in their canonical spelling**: decoders MUST reject a unit whose key field does not decompress, and MUST reject one whose key field is not its own point's re-compression — a masked `y` at or above the field prime, or the negative-zero encoding (`x = 0` with the sign bit set) that RFC 8032 §5.1.3 assigns to no point — even where that field is never verified against within the unit. (A 32-byte string that cannot denote a key is not the canonical encoding of anything, and an alternate spelling of a valid key lets one key travel as two byte strings, so byte-keyed equality, dedup at a serial tie, and digest stability each read one statement as two. Implementations that disagree about whether such a unit exists diverge on every derivation it feeds — the parser-differential class this document exists to kill. Both rejections also apply to the TXT record's `g=` and `p=` fields. One key, one spelling. Lenient decompression in a signature-*verification* step of a foreign protocol — e.g. RFC 8080 DNSKEY material, whose spelling is covered by the parent zone's signature — is outside this rule, which governs Onomancy's own wire formats.)
...
- NSEC/NSEC3 denial-of-existence records, where present, are framed like any other link — but verifiers skip them unverified: negative proofs are outside the protocol at v0.
