...
//! mandates three distinct dispositions, which [`TxtRecord::classify`]
//! makes unrepresentable to confuse:
//!
//! ```text
...
/// use onomancy_core::txt::TxtRecord;
///
...
/// # Ok::<_, onomancy_core::txt::ParseTxtRecordError>(())
/// ```
...
impl TxtRecord {
    ...
    pub fn classify(raw: &str) -> Result<Classified, ParseTxtRecordError> {
        ...
        let version_digits = version_onward
            .split(';')
            .next()
            .unwrap_or(version_onward);
...
impl fmt::Display for TxtRecord {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            ...
            self.serial,
            BASE64.encode(self.generation.0.as_bytes()),
...
///
/// See the [module docs](self) for the mapping. `Binding` boxes its
/// record: dispositions are transient classification results, and the
/// two key-bearing fields make the record large relative to the unit
/// variants.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum Classified {
    /// A strictly conforming `ONO0` binding record.
    Binding(alloc::boxed::Box<TxtRecord>),
...
///
/// A distinct newtype from [`DocAnchor`] on purpose — generation keys
/// and document IDs are both ed25519 keys, and confusing them is
/// exactly the kind of bug newtypes exist to make unrepresentable.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
...
fn decode_key_field(
    ...
    let key_bytes: [u8; KEY_LEN] = bytes.as_slice().try_into().map_err(|_| {
        ParseTxtRecordError::WrongKeyLength {
            field: name,
            got: bytes.len(),
        }
    })?;
...
mod tests {
    ...
    /// classify is total: arbitrary bytes never panic, and only `ONO0`
    /// inputs can produce errors.
    #[test]
    fn prop_classify_is_total_and_errors_only_on_ono0() {
    ...
    #[test]
    fn prop_accepted_strings_are_canonical() {
        bolero::check!().for_each(|bytes: &[u8]| {
            let Ok(raw) = core::str::from_utf8(bytes) else {
                return;
            };
    ...
    fn serial_grammar_is_pinpointed() {
        ...
        assert_eq!(Serial::parse("1_0"), Err(ParseSerialError::NotADigit { at: 1 }));
        assert_eq!(
